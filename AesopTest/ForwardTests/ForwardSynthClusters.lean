...
/-
Note this test is done with `Nat` as it is hard to create distinct clusters over `SNat`
...
elab "testCluster " nPremises:num nRules:num nClusters:num " by " ts:tacticSeq : command => do
  ...
  let binders : TSyntaxArray ``Term.bracketedBinder ←
    dArray.mapIdxM fun i pNameN ↦ do
      `(bracketedBinder| ($(mkIdent $ .mkSimple $ "BM" ++ toString nIter ++ "p" ++ toString i) :
     ...
     dArray.mapIdxM fun i pNameN ↦ do
      `(bracketedBinder| ($(mkIdent $ .mkSimple $ "BM" ++ toString nIter ++ "p" ++ toString i) :
         $(mkIdent pNameN.1):ident $(Lean.Syntax.mkNatLit (pNameN.2))
          ($(Lean.Syntax.mkNatLit (pNameN.2 + 1)))))
  elabCommand $ ← `(command|
    theorem $(mkIdent $ .mkSimple $ "BM.t" ++ toString nIter) $binders:bracketedBinder*
      : True := by $ts
  )
