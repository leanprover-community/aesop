...
def evalForward : Tactic
  | `(tactic| forward $t:ident $[[ $immediate:ident* ]]?) =>
    liftMetaTactic λ goal => do
      let e := mkFVar (← getLocalDeclFromUserName t.getId).fvarId
      let immediate := immediate.map (·.map (·.getId))
      let immediate ← getImmediateNames e immediate
      return [← RuleTac.applyForwardRule goal e immediate]
  | _ => unreachable!
