...
This is the anonymous supplement for the paper "Aesop: White-Box Best-First
Proof Search for Lean", submitted to CPP 2023.

## Building Aesop

Aesop is a regular Lean 4 library, so the official setup instructions at
<https://leanprover.github.io/lean4/doc/setup.html> apply. To summarise:

- Install [elan](https://github.com/leanprover/elan). This is a tool which
  manages installations of the Lean toolchain. After the installation, there
  should be a `lake` binary on your `PATH`. (`lake` is Lean's build tool and
  package manager.)
- In the directory where this README is located, run `lake build`. This will
  - download the appropriate version of Lean;
  - download the appropriate versions of Aesop's dependencies;
  - build these dependencies;
  - finally, build Aesop.
  There should be no errors during this process. There may be warnings, e.g.
  about deprecations or unused arguments. These can be ignored.

Lean officially supports x86-64 Linux, MacOS and Windows. Other platforms,
particularly M1 MacOS, may or may not work.

## Running Tests

Aesop's tests are located in the `tests/run` and `tests/golden` directories. To
run the full test suite, run `./test` in the directory where this README is
located. (This assumes a Unix-like system, so on Windows you may want to use the
Windows Subsystem for Linux.) This command type-checks all the test files. All
tests should succeed without any output except for the file
`tests/run/List.lean`, which should print a number of warnings of the form

``` text
tests/run/List.lean:968:8: warning: declaration uses 'sorry'
```

To run a single test, use

``` text
./test tests/<dir>/<file>.lean
```

or

``` text
lake env lean tests/<dir>/<file>.lean
```

The latter should work on Windows as well.

## Evaluating Case Studies

### Lists

The list case study from Sec. 5.1 is based on the test file
`tests/run/List.lean`. The numbers quoted in the paper are generated by the
script `evaluate-list-case-study`.

### Sequent Calculus Prover

The sequent calculus prover case study from Sec. 5.2 is based on the test file
`tests/run/SeqCalcProver.lean`. All code mentioned in the paper should be
present in this file.

## Exploring the Code

All Aesop code is located under the `Aesop/` directory. The main entry point is
either `Aesop/Main.lean`, which defines the `aesop` tactic, or the `bestFirst`
function in `Aesop/Search/Main.lean`, which implements the underlying search
procedure.

The following list provides a mapping from sections of the paper to the
corresponding parts of the implementation.

- Sec. 2.1 and 2.7: a rule (or multi-rule) is defined by two data structures, a
  `RuleTac` (from `Aesop/RuleTac/Basic.lean`), which is a tactic with some
  additional metadata, and a `Rule` (from `Aesop/Rule/Basic.lean`), which
  bundles a `RuleTac` with yet more metadata.
- Sec. 2.2 and 4.2: the search tree data structure is defined in
  `Aesop/Tree/Data.lean`. Functions to propagate the node states are in
  `Aesop/Tree/State.lean`. Functions to check various invariants are in
  `Aesop/Tree/Check.lean`.
- Sec. 2.3--2.5: the main search loop is in `Aesop/Search/Main.lean`. Expansion
  -- the most interesting part of the search algorithm -- is
  `Aesop/Search/Expansion.lean`.
- Sec. 2.6: the function which extracts the safe goals from a search tree is in
  `Aesop/Tree/ExtractProof.lean`. The function which expands safe rules that
  have not been expanded during the search is in
  `Aesop/Search/ExpandSafePrefix.lean`.
- Sec. 3.1: each rule builder is defined by two structures: a `RuleTac` which
  implements the rule's behaviour, defined in `Aesop/RuleTac/<Builder>.lean`
  (e.g. `Aesop/RuleTac/Cases.lean`), and the actual builder which turns a
  declaration into a rule, defined in `Aesop/Builder/<Builder>.lean`.
- Sec. 3.2: the builtin simplifier invocation is performed by the function
  `normSimp` in `Aesop/Search/Expansion.lean`.
- Sec. 3.3: the rule index is implemented in `Aesop/Index/Basic.lean` and
  `Aesop/Index.lean`.
- Sec. 3.4: the default rules (called "builtin rules" in the codebase) are
  implemented in `Aesop/BuiltinRules.lean` and in various files under the
  `Aesop/BuiltinRules` directory.
- Sec. 4.3 and 4.6: the function which adds a new rapp to the tree (and which,
  in the process, copies m-coupled goals and adds subgoals for dropped
  metavariables as necessary) is implemented in `Aesop/Tree/AddRapp.lean`.
- Sec. 4.4: safe rapp postponing is implemented in
  `Aesop/Search/Expansion.lean`, particularly the `runFirstSafeRule` function.

--------------------------------------------------------------------------------

The original Aesop README follows.
...
- **`forward`**: when applied to a declaration or hypothesis of type `A₁ → ...
  ...
  When a `forward` rule has been successfully applied, it will not be applied
  again to the same hypotheses when we expand its subgoals. Otherwise every
  `forward` rule would apply infinitely often.
